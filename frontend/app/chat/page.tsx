...
export default function ChatPage() {
  ...
  // ←---- Only this function has been replaced ----→
  const handleSend = async () => {
    ...
    // 2) Try the built‐in "/api/ask", fall back to Flask if 404
    let res: Response;
    try {
      ...
      if (res.status === 404) {
        res = await fetch(`${LOCAL_BACKEND}/api/ask`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ question: userMessage.text }),
        });
      ...
      console.error('Fetch error, even after fallback:', err);
      const errorMsg: Message = {
        id: messages.length + 2,
        text: '😢 Oops—could not reach the AI right now.',
        sender: 'ai',
        timestamp: new Date(),
      };
